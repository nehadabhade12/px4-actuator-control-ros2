...
class FinalHover(Node):
    ...
    def odom_cb(self, msg):
        ...
        consistency_error = abs(z_raw - z_pred)
        sign_flip = (self.z_est < -0.5 and z_raw > -0.2)

        if consistency_error > 0.8 or sign_flip:
            self.z_est = z_pred
            self.ekf_bad = True
            return
    ...
    def loop(self):
        ...
        corr_pitch = -(K_att * self.pitch + D_att * d_pitch)
        corr_yaw = -(K_yaw * self.yaw)

        corr_roll = max(-0.05, min(0.05, corr_roll))
        corr_pitch = max(-0.05, min(0.05, corr_pitch))
        corr_yaw = max(-0.03, min(0.03, corr_yaw))
        ...
        if self.counter % 50 == 0:
            self.get_logger().info(
                f"z={self.z_est:.2f}, vz={self.vz_est:.2f}, T={T:.3f}"
            )
